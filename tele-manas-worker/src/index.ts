...
export default {
  // Runs daily at 03:00 UTC via the cron trigger in wrangler.toml
...
async function handleTrigger(env: Env): Promise<Response> {
  try {
    ...
    return Response.json({
      ...
      facilitiesKey: result.facilitiesKey,
...
async function handleHealth(env: Env): Promise<Response> {
  const [api, r2] = await Promise.all([checkApi(), checkR2(env.TELE_MANAS_BUCKET)]);
...
async function checkR2(bucket: R2Bucket): Promise<{ ok: boolean; error?: string }> {
  try {
